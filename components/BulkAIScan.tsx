...
export default function BulkAIScan({ cafeId, cafeSlug, existingCategories }: Props) {
  ...
  function addFiles(selected: File[]) {
    ...
    setFiles((prev) => {
      const combined = [...prev, ...valid].slice(0, 15);
      return combined;
    });
  }
        ...
        files.map(async ({ file }) => {
          ...
          return new Promise<string>((resolve, reject) => {
            ...
            reader.onload = () => {
              ...
              // data:image/jpeg;base64,XXX → sadece XXX
  ...
  function updateProduct(index: number, field: keyof ScannedProduct, value: string) {
  ...
  async function handleSaveAll() {
    ...
    if (missing !== -1) {
      setError(`${missing + 1}. ürünün fiyatı eksik.`);
      return;
    }
    setSaving(true);
    setError("");
    ...
    try {
      // Görselleri Storage'a yükle + ürünleri Supabase'e kaydet
      ...
      // Yeni kategoriler varsa cafe'ye de kaydet
      ...
      if (newCats.length > 0) {
        await supabase
          .from("cafes")
          .update({ category_order: [...currentOrder, ...newCats] })
          .eq("id", cafeId);
      }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-br from-orange-50 to-amber-50 border border-orange-100 rounded-2xl p-5 space-y-2">
        ...
        <p className="text-sm text-gray-600 leading-relaxed">
          ...
          <br />
          <span className="text-gray-500">Kategori ve fiyatı sen belirlersin.</span>
        </p>
      </div>

      {/* Upload alanı — sadece ürün taranmamışsa göster */}
      {products.length === 0 && (
        <div
          onDrop={handleDrop}
            ...
            {files.map(({ preview }, i) => (
              <div key={i} className="relative aspect-square rounded-xl overflow-hidden bg-gray-100">
                ...
                <button
                  ...
                  className="absolute top-1.5 right-1.5 bg-black/50 rounded-full p-1 hover:bg-black/70 transition-colors"
                >
          ...
          >
            {scanning ? (
              ...
              <>
                ...
                {files.length} Görseli Analiz Et
              </>
            )}
          </button>
        </div>
      ...
      {products.length > 0 && (
        <div className="space-y-5">
          <p className="text-sm text-gray-500 font-medium">
            {products.length} ürün analiz edildi — kategori ve fiyatı girerek kaydet.
          </p>
                    ...
                    <div className="flex-1 relative">
                      {newCatInputIndex === i ? (
                        <div className="flex gap-1">
                          ...
                          />
                          <button
                            onClick={() => handleAddNewCategory(i)}
                            className="bg-orange-500 text-white rounded-xl px-2.5 py-2"
                          >
